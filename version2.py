#!/usr/bin/env python3
"""
Enhanced RAG-Powered Argo Query System - WITH THINKING-BASED SQL GENERATION
...
class EnhancedArgoRAGSystem:
    """Enhanced RAG system with thinking-based SQL generation"""
...
Provide a comprehensive, detailed answer based on the knowledge base above."""
    ...
    def generate_thinking_sql(self, user_query: str, show_details: bool = False) -> Dict[str, Any]:
        """✅ NEW: Generate SQL using thinking-based approach"""
        
        if show_details:
            print(f"\n🧠 Generating THINKING-based SQL for: '{user_query}'")
        ...
        # ✅ ENHANCED: Thinking-based system prompt
        system_prompt = f"""You are an expert PostgreSQL query generator and oceanographic data analyst for Argo float databases.

BEFORE generating SQL, follow this systematic THINKING PROCESS:

🔍 **1. QUERY ANALYSIS**:
- What is the user asking for?
- What oceanographic concept or relationship is involved?
...
- What joins or relationships are needed?
- Are there quality control considerations?

🧠 **3. LOGIC PLANNING**:
- Break down the query into logical steps
- Identify complex conditions or calculations needed
- Plan WHERE clauses and filters
- Consider oceanographic constraints (depth ranges, regions, etc.)
- For comparisons: define what constitutes a valid comparison

💻 **4. SQL CONSTRUCTION**:
- Build the query systematically
- Include appropriate quality control filters
- Ensure logic matches the analytical intent
- Use proper PostgreSQL syntax

CRITICAL RULES:
- platform_number is VARCHAR → ALWAYS use quotes: '13859' NOT 13859
- Pressure ≈ depth: 1 dbar ≈ 1 meter depth
- Quality filters: temp_qc IN ('1', '2') for good data, psal_qc IN ('1', '2') for salinity
- For depth comparisons: use self-joins on same platform_number + cycle_number
- For inversions: deeper water (higher pressure) warmer than shallow water (lower pressure)
- For regional analysis: use latitude/longitude bounds
- DEPTH CONSTRAINTS: "upper 100m" → pres <= 100, "below 500m" → pres >= 500

EXAMPLES OF COMPLEX LOGIC:
- Temperature inversions: Compare temp at different pressures within same profile, find where deep_temp > shallow_temp
- Regional queries: Use geographic bounds (lat/lon ranges for ocean basins)
- Multi-table joins: Use platform_number as foreign key between tables

USER QUERY: {user_query}

Now work through the THINKING PROCESS above step by step, then generate the final SQL query.

RESPONSE FORMAT:
First show your thinking process, then provide the SQL query in a code block."""
        ...
        payload = {
            "model": "sonar",
            "messages": messages,
            "temperature": 0.1,  # Lower temperature for more systematic thinking
            "max_tokens": 1000    # More tokens for thinking process
        }
        ...
        try:
            if show_details:
                print("🌐 Calling Perplexity API for thinking-based SQL generation...")
            ...
            # Extract SQL from the thinking response
    ...
    def _extract_sql(self, response: str) -> str:
        """✅ IMPROVED: Extract clean SQL from thinking response"""
        ...
        # Method 2: Look for SELECT or WITH statements after thinking process
        sql_statement_patterns = [
            r"SQL CONSTRUCTION:.*?(WITH\s+\w+\s+AS\s*\(.*?SELECT.*?)(?:;|$|\n\n|$)",
            r"SQL CONSTRUCTION:.*?(SELECT.*?)(?:;|$|\n\n)",
            r"Final SQL.*?:(.*?)(SELECT.*?)(?:;|$|\n\n)",
        ...
        for pattern in sql_statement_patterns:
            match = re.search(pattern, response, re.DOTALL | re.IGNORECASE)
            if match:
                # Get the last capture group (the SQL part)
                sql = match.group(-1).strip()
                # Validate it looks like SQL
                if sql.upper().startswith(('SELECT', 'WITH')) and any(keyword in sql.upper() for keyword in ['FROM', 'WHERE', 'JOIN']):
                    return sql
        
        # Method 3: Look for any SQL-like content after keywords
        sql_keywords = ['SQL Query:', 'Final Query:', 'Query:', '4. SQL CONSTRUCTION:']
        for keyword in sql_keywords:
            if keyword in response:
                after_keyword = response.split(keyword, 1)[1]
                ...
                if sql_lines:
                    start_idx = after_keyword.find(sql_lines[0])
        ...
        # If all else fails, return the first SQL-like statement found
        match = re.search(r'((?:SELECT|WITH).*?)(?:;|$|\n\n)', response, re.DOTALL | re.IGNORECASE)
        if match:
            return match.group(1).strip()
...
8. CRITICAL: For temperature inversions, ensure you understand that an inversion means DEEPER water is WARMER than SURFACE water
...
Please format this data into a clear, informative response for the user."""}
    ...
    def process_enhanced_query(self, user_query: str, show_details: bool = False) -> Dict[str, Any]:
        """Enhanced query processing with thinking-based SQL generation"""
        ...
        if query_type == 'general':
            ...
            # Handle technical database queries with THINKING approach
            if show_details:
                print("🗄️ Processing as technical database query with THINKING...")
            
            # ✅ CHANGED: Use thinking-based SQL generation
            sql_result = self.generate_thinking_sql(user_query, show_details)
...
def interactive_enhanced_chat():
    """🎯 ENHANCED INTERACTIVE CHAT INTERFACE WITH THINKING"""
    
    PERPLEXITY_API_KEY = "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"
    
    print("\n🌊 ENHANCED ARGO RAG SYSTEM - WITH THINKING-BASED SQL GENERATION")
    print("=" * 75)
    ...
    try:
        ...
        print("\n🧠 Enhanced RAG System with Thinking Ready!")
        print("\n📝 You can now ask:")
        ...
        print("   • Find temperature inversions where deeper water is warmer than surface water")
        ...
        print("\n💡 **Commands**:")
        print("   • 'detailed' - Toggle detailed processing output (shows thinking process)")
        ...
        while True:
            ...
            try:
                # Process enhanced query with thinking
                ...
                if result['success']:
                    ...
                    if 'sql_query' in result:
                        print(f"\n💻 **Attempted SQL**: {result['sql_query']}")
